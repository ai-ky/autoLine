...
def bring_to_front():
    ...
    if now.hour == selected_hour and now.minute == selected_min and not isDone:
        log(f"[{now.strftime('%H:%M:%S')}] time's up！\n")
        show_window()
        isDone = True
    ...
    if autoline.hideWindow(group_title) is None:
        log(f"can't find window [{group_title}]\n")
...
def setup_tray():
    ...
    icon = pystray.Icon("AutoLine", icon_image, "AutoLine", menu=pystray.Menu(
        pystray.MenuItem("display", on_show),
...
root.title("dayly auto message routine")

main_frame = tk.Frame(root)
main_frame.pack(fill='both', expand=True, padx=10, pady=10)

# 群組視窗標題
tk.Label(main_frame, text="title of the chat group：", anchor='w').pack(anchor='w')
title_var = tk.StringVar(value="LINE")
tk.Entry(main_frame, textvariable=title_var).pack(fill='x', pady=(0, 10))

# 推播時間
tk.Label(main_frame, text="target time：", anchor='w').pack(anchor='w')
...
chk_daily = tk.Checkbutton(main_frame, text="every day", variable=daily_var, anchor='w', justify='left')
chk_holiday = tk.Checkbutton(main_frame, text="include holiday", variable=holiday_var, anchor='w', justify='left')
chk_daily.pack(anchor='w')
chk_holiday.pack(anchor='w')

# 訊息欄（唯讀 + 捲軸）
tk.Label(main_frame, text="log message：", anchor='w').pack(anchor='w', pady=(10, 0))
...
# 下拉選單：檢查間隔
tk.Label(main_frame, text="checking duration（sec）：", anchor='w').pack(anchor='w', pady=(10, 0))
